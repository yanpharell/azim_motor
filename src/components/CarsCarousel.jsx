...
export default function CarsCarousel({ cars, onReserveClick }) {
  ...
  return (
    <div className="relative w-full overflow-hidden">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {visibleCars.map((car, index) => (
              <CarCard key={index} car={car} onReserveClick={onReserveClick} />
            ))}
          </motion.div>
        </AnimatePresence>
      </div>

      {/* Navigation Buttons - Afficher uniquement s'il y a des voitures */}
      {cars.length > 0 && (
        <button
          onClick={prevSlide}
      ...
      {/* Indicators - Afficher uniquement s'il y a des voitures */}
      {cars.length > 0 && (
